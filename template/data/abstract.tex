...
comparisons, with MNIST used as a lightweight sanity benchmark. In the
three-seed CIFAR-10 alpha sweep, AdaGrad-OTA and Adam-OTA substantially
outperform FedAvg-OTA and FedAvgM-OTA under the implemented heavy-tail
setting, although the observed alpha trend is not monotonic and the MAC
comparison is neutral. The project is therefore not a full line-by-line
reproduction of the reference paper, but it implements the core heavy-tail
setting needed to evaluate a carefully scoped adaptive OTA-FL robustness
claim.
